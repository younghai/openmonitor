...
window.addEventListener(
  ...
  async (event) => {
    ...
    if (target && (target.tagName === "SCRIPT" || target.tagName === "LINK")) {
      ...
      // Only check for Vtstirele's own chunks (relative URLs or same origin)
      // Ignore external CDN resources (absolute URLs with different origins)
      const isVtstireleResource = (() => {
        try {
          ...
          // Invalid URL - assume it's a relative path (Vtstirele resource)
          return true;
        }
      })();

      if (isVtstireleResource) {
