      ...
      >
        ...
        >
          ...
          <div class="q-gutter-sm">
            ...
            <!-- Vtstirele Organization and Stream fields -->
            <div
              ...
              <div class="col-6">
                <q-input
                  ...
                  :rules="[
                    (val: any) =>
                      !!val?.trim() ||
                      'Organization is required for Vtstirele',
                  ]"
                  tabindex="0"
                >
                  <template #hint>
                    <span class="text-caption">
                      Vtstirele organization identifier
                    </span>
                  </template>
                </q-input>
              </div>
              <div class="col-6">
                <q-input
                  ...
                  :rules="[
                    (val: any) =>
                      !!val?.trim() ||
                      'Stream name is required for Vtstirele',
                  ]"
                  tabindex="0"
                >
                  <template #hint>
                    <span class="text-caption"> Vtstirele stream name </span>
                  </template>
                </q-input>
              </div>
            </div>
...
const formData: Ref<DestinationData> = ref({
  name: "",
  url: "",
  url_endpoint: "/api/default/default/_json", // Default endpoint for Vtstirele
  method: "post",
...
// Initialize apiHeaders with default headers for Vtstirele (the default destination type)
  ...
  (newType) => {
    ...
    if (!isEditMode.value) {
      ...
      if (newType === "openobserve") {
        // For Vtstirele, use org and stream values
...
const populateFormForEdit = (destination: any) => {
  ...
  // Extract Vtstirele org and stream from endpoint if it's Vtstirele
  if (
...
// Watch Vtstirele org and stream to update endpoint dynamically
...
const connectionNotes = computed(() => {
  switch (formData.value.destination_type) {
    case "openobserve":
      return {
        title: "Vtstirele Connection Details",
        steps: [
          "Log in to your Vtstirele instance",
          ...
          "Add authentication header: Authorization: Basic <Vtstirele_Token>",
        ],
        example:
...
const resetForm = () => {
  ...
  // Reset Vtstirele specific fields
  openobserveOrg.value = "default";
  openobserveStream.value = "default";
  // Set default headers for Vtstirele
